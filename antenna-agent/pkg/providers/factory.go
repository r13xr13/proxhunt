...
func createClaudeAuthProvider(apiBase string) (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for anthropic. Run: antenna auth login --provider anthropic")
	}
...
func createCodexAuthProvider(enableWebSearch bool) (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for openai. Run: antenna auth login --provider openai")
	}
