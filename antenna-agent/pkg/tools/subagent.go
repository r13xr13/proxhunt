...
func (sm *SubagentManager) Spawn(ctx context.Context, task, label, agentID, originChannel, originChatID string, callback AsyncCallback) (string, error) {
	...
	default:
		...
		if label != "" {
			switch {
			case strings.Contains(strings.ToLower(label), "research"):
				model = "ollama/llama3.2:1b"
			case strings.Contains(strings.ToLower(label), "code"):
				model = "ollama/codellama:7b"
			case strings.Contains(strings.ToLower(label), "write"):
				model = "ollama/llama3.2"
			}
		}
	}
...
After completing the task, provide a clear summary of what was done.`
	...
	// Use task-specific model or default
	model := task.Model
	if model == "" {
		model = sm.defaultModel
	}
