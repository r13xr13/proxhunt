...
Antenna's tools configuration is located in the `tools` field of `config.json`.
...
By default, Antenna blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `PICOPRAWN_TOOLS_<SECTION>_<KEY>`:

For example:
- `PICOPRAWN_TOOLS_WEB_BRAVE_ENABLED=true`
- `PICOPRAWN_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `PICOPRAWN_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
